...
#pragma region Section: Appearances
  if (ImGui::CollapsingHeader (u8"外观###SKIF_SettingsHeader-1"))
  {
    ...
    if ( ImGui::Checkbox ( u8"记住最后选择得游戏",           &_registry.bRememberLastSelected ) )
...
#pragma region Section: Advanced
  if (ImGui::CollapsingHeader (u8"高级###SKIF_SettingsHeader-3"))
  {
    ImGui::PushStyleColor   (
      ImGuiCol_Text, ImGui::GetStyleColorVec4(ImGuiCol_SKIF_TextBase)
                              );
...
#pragma region Section: Whitelist / Blacklist
  if (ImGui::CollapsingHeader (u8"白名单 / 黑名单###SKIF_SettingsHeader-4")) //, ImGuiTreeNodeFlags_DefaultOpen)) // Disabled auto-open for this section
  {
...
#pragma region Section: SwapChain Presentation Monitor
  if (ImGui::CollapsingHeader (u8"SwapChain 表示监视器###SKIF_SettingsHeader-6", ImGuiTreeNodeFlags_DefaultOpen))
  {
    ImGui::PushStyleColor (
      ImGuiCol_Text, ImGui::GetStyleColorVec4(ImGuiCol_SKIF_TextBase)
                            );
